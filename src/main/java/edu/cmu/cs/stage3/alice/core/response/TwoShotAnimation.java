...
public class TwoShotAnimation extends AbstractPositionAnimation {
	...
	public class RuntimeTwoShotAnimation extends RuntimeAbstractPositionAnimation {
			...
			public void prologue( double t ) {
				...
				super.prologue(t);
				m_asSeenBy2 = TwoShotAnimation.this.asSeenBy2.getReferenceFrameValue();
				m_amount = TwoShotAnimation.this.amount.getNumberValue().doubleValue();
				
//				CLK - begin	
				  if( m_asSeenBy2 == null ) {
					  throw new edu.cmu.cs.stage3.alice.core.SimulationPropertyException( "second character value must not be null.", getCurrentStack(), TwoShotAnimation.this.asSeenBy2 );            
				  }
				  if( m_subject == null ) {
				  ...
				  if( m_asSeenBy2 == m_asSeenBy ) {
					  throw new edu.cmu.cs.stage3.alice.core.SimulationPropertyException( "first and second characters must be different", getCurrentStack(), TwoShotAnimation.this.asSeenBy2 );            
				  }
				  ...
				  if (m_amount < 0) {
					  throw new edu.cmu.cs.stage3.alice.core.SimulationPropertyException( "amount must be greater than 0", getCurrentStack(), TwoShotAnimation.this.amount );            
				  }
				  //CLK - end
			...
			protected javax.vecmath.Vector3d getPositionEnd() {
				...
				edu.cmu.cs.stage3.alice.core.SpatialRelation sv = TwoShotAnimation.this.spatialRelation.getSpatialRelationValue();
				...
				// ok, at this point, I have a camera orientation and an initial position.
				// for the offset positions (front-right etc), I need to calculate the correct initial camera
				// positions. For the others, I just need to figure out how far back to move the camera
				...
				//if (!sv.equals(edu.cmu.cs.stage3.alice.core.SpatialRelation.ABOVE) && !sv.equals(edu.cmu.cs.stage3.alice.core.SpatialRelation.BELOW)) {
					...
					largestDimSize =  TwoShotAnimation.this.amount.doubleValue() * m_asSeenByBoundingBox.getHeight();
					...
					double halfViewable = (m_asSeenByBoundingBox.getHeight() * TwoShotAnimation.this.amount.doubleValue() * 1.5)/ 2.0;
					double startingHeight = (1.0 - TwoShotAnimation.this.amount.doubleValue()) * m_asSeenByBoundingBox.getHeight();
					...
					if ( TwoShotAnimation.this.amount.doubleValue() * m_asSeenByBoundingBox.getHeight() > largestDimSize) {
						largestDimSize =  TwoShotAnimation.this.amount.doubleValue() * m_asSeenByBoundingBox.getHeight();
					}
					...
					double halfViewable = (m_asSeenByBoundingBox.getHeight() * 1.5 * TwoShotAnimation.this.amount.doubleValue())/ 2.0;
					double startingHeight = (1.0 - TwoShotAnimation.this.amount.doubleValue()) * m_asSeenByBoundingBox.getHeight();
					m_cameraHeight = startingHeight + halfViewable + m_asSeenByBoundingBox.getMinimum().y;
				...
				if ( (sv.equals(edu.cmu.cs.stage3.alice.core.SpatialRelation.LEFT_OF)) || (sv.equals(edu.cmu.cs.stage3.alice.core.SpatialRelation.RIGHT_OF)) ) {
					if ( TwoShotAnimation.this.amount.doubleValue() * m_asSeenByBoundingBox.getHeight() > m_asSeenByBoundingBox.getDepth()) {
						largestDimSize = m_asSeenByBoundingBox.getHeight();
						v = sv.getPlaceVector( largestDimSize * 2.0 * TwoShotAnimation.this.amount.doubleValue(), m_subjectBoundingBox, m_asSeenByBoundingBox );
					} else {
						largestDimSize = m_asSeenByBoundingBox.getDepth();
						v = sv.getPlaceVector( largestDimSize * 2.0, m_subjectBoundingBox, m_asSeenByBoundingBox );
					}
				} else {
					if ( TwoShotAnimation.this.amount.doubleValue()* m_asSeenByBoundingBox.getHeight() > m_asSeenByBoundingBox.getWidth()) {
						largestDimSize = m_asSeenByBoundingBox.getHeight();
						v = sv.getPlaceVector( largestDimSize * 2.0 * TwoShotAnimation.this.amount.doubleValue(), m_subjectBoundingBox, m_asSeenByBoundingBox );
					} else {
						largestDimSize = m_asSeenByBoundingBox.getWidth();
				...
				double halfViewable = (m_asSeenByBoundingBox.getHeight() * TwoShotAnimation.this.amount.doubleValue())/ 2.0;
				double startingHeight = (1.0 - TwoShotAnimation.this.amount.doubleValue()) * m_asSeenByBoundingBox.getHeight();
				m_cameraHeight = startingHeight + halfViewable + m_asSeenByBoundingBox.getMinimum().y;
				
				v.y = m_cameraHeight;
				
				javax.vecmath.Vector3d center = m_asSeenByBoundingBox.getCenter();
			...
			public void update( double t ) {
				// todo: this probably just wants to be interpolating the quaternion and shouldn't
				...
				//javax.vecmath.Vector3d center = m_asSeenByBoundingBox.getCenter();
				//m_subject.pointAtRightNow(m_asSeenBy, new javax.vecmath.Vector3d(center.x,m_cameraHeight,center.z), new javax.vecmath.Vector3d(0,1,0), m_asSeenBy, false);
