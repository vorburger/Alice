...
public class WalkToAnimation extends AbstractWalkAnimation {
	...
	public class RuntimeWalkToAnimation extends RuntimeAbstractWalkAnimation {
		...
		protected javax.vecmath.Vector3d getPositionEnd() {
			...
			edu.cmu.cs.stage3.alice.core.SpatialRelation sv = WalkToAnimation.this.spatialRelation.getSpatialRelationValue();
			javax.vecmath.Vector3d v = sv.getPlaceVector( WalkToAnimation.this.amount.doubleValue(), m_subjectBoundingBox, m_asSeenByBoundingBox );
			return v;
		}
		...
		protected double getPathLength() {
			...
			//System.out.println("distance between points: " + java.lang.Math.sqrt((m_transformationEnd.m30 - m_transformationBegin.m30) * (m_transformationEnd.m30 - m_transformationBegin.m30) + (m_transformationEnd.m32 - m_transformationBegin.m32) * (m_transformationEnd.m32 - m_transformationBegin.m32)) );
		...
		public void prologue( double t ) {
			...
			m_asSeenBy = WalkToAnimation.this.asSeenBy.getReferenceFrameValue();
			...
			if( m_asSeenBy == null ) {
                throw new edu.cmu.cs.stage3.alice.core.SimulationPropertyException( subject.name.getStringValue() + " needs something or someone to walk to.", null, WalkToAnimation.this.asSeenBy );
            }
            if( subject == m_asSeenBy ) {
                throw new edu.cmu.cs.stage3.alice.core.SimulationPropertyException( subject.name.getStringValue() + " can't walk to " + subject.name.getStringValue() + ".", getCurrentStack(), WalkToAnimation.this.asSeenBy );            
            }
            
            if (subject.isAncestorOf(m_asSeenBy)) {
            	throw new edu.cmu.cs.stage3.alice.core.SimulationPropertyException( subject.name.getStringValue() + " can't walk to a part of itself", getCurrentStack(), WalkToAnimation.this.asSeenBy );            
            }
			...
			//javax.vecmath.Matrix3d paMatrix = subject.calculatePointAt(m_asSeenBy, null, new javax.vecmath.Vector3d(0,1,0), null, true);
		...
		public void update( double t ) {		
			if (this.getTimeRemaining(t) > 0) {
				...
				if (portion <= 1.0) {
					...
					//int stepNumber = (int)java.lang.Math.ceil( getTimeElapsed(t)  * stepSpeed.doubleValue()) - 1;
